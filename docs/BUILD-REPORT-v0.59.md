...
| 8 | fewer mushrooms; plumes at half | **FESTIVAL COST ONLY** — Jerry clarified this after the round shipped and it is what shipped, now asserted rather than merely true. `FESTIVAL_MUSHROOMS` and `FESTIVAL_PLUME_SHARE` are read in `festivalCost()` and nowhere else; every other mushroom/plume price is untouched (Noxus 120, raptors 12–18, Jessed Hawks 120, Beast Lore 30/40, Harvest Rites 400, Shaco 60, Twitch 90) and the per-head luxury drain is still identical across furs, mushrooms and plumes. **The Festival's own** mushroom line goes `4 → 2` per unit of comfort and its plume line is **derived as half of that line**, so the ratio cannot drift. At `luxuryComfort() = 3`: mushrooms 12 → 6, plumes 12 → 3. |
...
| 15 | All eight feel notes have a ledger row | | **PASS** — eight rows plus two for Jerry's dev notes. Note 8's row was re-scoped after Jerry clarified that "fewer mushrooms / plumes at half" is a FESTIVAL-COST change only; the code always was, and three new `test-v59` assertions now pin the scope in both directions. |

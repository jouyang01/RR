...
| | |
|---|---|
| Last shipped build | **v0.64**, tagged `v0.64` — **THE GATE CLEARED.** Icathia on **3 of 3** seeds and peak population **180 median with all three seeds inside §27's 150–220 band**, after three rounds of failing both. The housing ladder decoupled from the food ceiling, Option B's rails shipped on Jerry's ruling, and `DISCOVERY_RUNG_CAP` retired. |
...
| **THE KNEE AUDIT — ZERO FAMILIES PAST THE KNEE, for the first time** | Option B shipped on Jerry's ruling: **vigor 1.0 → 8.0, devotion 2.0 → 5.0, provisions 1.5 → 3.0, mana 1.0 → 2.0**; crystals, gold and culture untouched because all three already delivered in full. End of run every family throws away **0%**, against vigor's 82.1% and devotion's 46.4% one round ago. The rail rule is `ceil(raw Σ ÷ 0.75)` and the source's pattern is Solar Revolution's limit 10 against a reachable ~4.5 (`js/religion.js:1548-1550`). **The rails are INERT before year 300** — no family reaches its old knee that early — which is the property the design was chosen for. |
| **A CONVERTER IS ANY BUILDING WITH A `convert` BLOCK — ruled v0.64** | `autoprod` says how a converter is DRIVEN, not whether it converts. Three converter-output categories (the conversion Discoveries, the overseer affinity, the Cinders buff) reached only the non-autoprod converters, so **Banked Coals raised the Forge and both Refineries and did nothing for the two buildings that make Zaun Ore and Coalgas — the inputs the Shimmer Refinery it DOES boost runs on.** v0.58.1 note 29 had made this argument for the drake in the same function and it was applied to one term only. **No new multiplicative category (§31); three existing terms got a wider scope.** |
| **`DISCOVERY_RUNG_CAP` IS RETIRED** | The per-RUNG total is **not** a Kittens invariant — range 0.30–8.19, and half the source's own rungs exceed 2.43×. **The per-UPGRADE ratio is**: IQR 0.73–1.00, median 0.87, and RR sits at **0.80** with every generated member exactly there. The cap had taken the generated members to **0.34×, far below the source's p25**. The two authored outliers were re-based by hand to p75: `greatLibrary` 40,000 → 12,000, `masterOfTheHunt` 12,000 → 3,600. **The census join is pinned to `tools/kittens-upgrade-census.mjs` and recorded at the site.** |
| **A TERM KEYED ON A NAME IS DEAD UNTIL THE CONTAINER DECLARES THAT NAME — new operational rule** | v0.63 Part 3.2's Demacian Accord (+8.5% timber and ore) is applied by `for (var pk in boosts)`, and `boosts` had no `timber` or `ore` key. **Measured on the v0.63 tag: +1.0%, which is `catPolicy`'s generic term, and 0.0% of the advertised figure.** Nothing threw, nothing rendered NaN, no suite failed, for a full version. **Assert the DELIVERED value, never the presence of the key.** |
| **THE UNDISCHARGED CONDITION — mana runs a measured deficit** | Net mana **−0.40/s at hexcore (ratio 1.003)** and **−14.52/s at end of run (ratio 1.011)**. The spec's pass condition 17 says ship a fourth mana discovery on a measured deficit; **the measurement arrived WITH the final gate**, so it is handed on rather than shipped unverified. Read BUILD REPORT §11.6 first — 0.3% and 1.1% of gross, and a ratio pinned at ≈1.00 is equilibrium rather than collapse. |
| **THREE FAILING CONDITIONS ARE ONE RANDOM VARIABLE** | Sparks **×1.98**, first champion **×1.85**, Chemtech→Hexcore **×2.70** — all three the 3-of-10 Piltover/Zaun champion draw (§4). **Builder note 5 predicted exactly this before the round ran.** Three seeds is not enough to steer on any of them. |
| **A LATE-GAME ROUND CANNOT BE ATTRIBUTED BY CHEAP PER-PART CHECKS** | **Seven of eight cumulative-prefix slices measured EXACTLY ZERO at 300 game-years.** A ceiling binding after y1,000, four rails whose families reach their knee in Era 3, era gates on `chemtech` — all invisible short. Either run the slices at full length or state up front that attribution comes from one ensemble. |
...
| Machine note | **`nproc` = 2.** This round's 3-seed 2,500-year ensemble took **87 minutes** while sharing the box with a slice chain. Budget 90–120. Launch with `setsid nohup … & disown`. |

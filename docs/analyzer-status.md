...
| | |
|---|---|
| Last shipped build | **v0.65**, tagged `v0.65` — **THE VIGOR ROUND, AND IT FOUND THAT VIGOR IS LOAD-BEARING FOR THE WHOLE ERA-3 ECONOMY.** Jerry's dev note deleted the Training Ground's vigor boost; Era-3 raw production fell **×4.7** and peak population **180 → 155**, with no Era-3 price touched. The Discovery ladder's knowledge coverage went **39 → 79 of 82** and *improved* pacing. |
| Previous build | **v0.64**, tagged `v0.64` — the gate cleared: Icathia 3 of 3, peak population 180 median inside §27's band, Option B's rails, `DISCOVERY_RUNG_CAP` retired |
| Last consumed spec | **`docs/specs/rr-analyzer-v065-spec.md`** (consumed by v0.65; moved out of the root) |
| Last consumed dev notes | **Jerry's one v0.65 note** — *"Training grounds should not increase Vigor generation"* — in `docs/gameplay-notes.md`. Shipped exactly as directed, and it is the largest measured change in the project's recorded history. |
| Current spec, awaiting a builder | **NONE.** No `current-build-spec.md` at the repo root. If one is present it is a leftover and this row wins. |
| **THE ROUND'S FINDING — vigor → expeditions → the Wilds → EVERYTHING** | `vigorPerSec` **82.16 → 31.4**. zaunore **587.8 → 126.1/s**, hexore **646.6 → 110.3/s**, coalgas **403.4 → 78.8/s**, mana gross **1,351 → 384/s**, trades **165,063 → 44,442**. Every housing tier at every milestone reads **stock-bound — timber**. **Vigor is the settlement's raw-material income, laundered through the Wilds**, and nothing in the project said so before this round measured it. **Whether that coupling is intended is Jerry's ruling and it blocks Era-3 tuning.** |
| **PART 1 — the Discovery ladder, and the spec's central prediction HELD** | Coverage **39 → 79 of 82**; total discovery knowledge **2,003,370**, whole-game ratio **1.3887** against the source's like-for-like 1.903; per-upgrade median **0.80** inside the source IQR 0.73–1.00. Predicted +150 to +500 later on Icathia, **+900 as the falsifier. Measured seed-for-seed: −328.9 / (never → completes) / +51.7 — median +51.7.** Discovery share of knowledge spend **6.7% → 59.7%** while time-at-cap **rose** 82.8% → 84.4%. **§24's reading of knowledge as a lumpy-sink-at-ceiling was describing a surplus, not a queue — confirmed.** |
| **THE GATE — Icathia FAILS at 4 of 5** | 2,033.1 / 2,434.7 / 1,731.2 / 1,896.1 / **never**. Peak population **PASSES at 155 median** (155 / 138 / 162 / 165 / 74). **Per the spec's own §1.6 the answer is the knowledge ceiling — the `libraryRatio` line, dated and unactioned since v0.56 — or `CONV_DISCOVERY_LINE`'s Σ0.65. NEVER a discovery price below the source's band.** |
| **SEED 5 IS A NEW KIND OF FAILURE** | Peak population **74**, **470 trades** against a median of 44,442, Era 3 never reached — **and it drew its first Piltover/Zaun champion EARLIEST of the five.** Every previous round's failures were attributable to the champion order. **A seed can now fail for a reason that is not the draw**, and three seeds would have missed it. **Five seeds is the new minimum.** |
| **PART 5 SEPARATED THE CHAMPION DRAW FROM THE DESIGN, for the first time** | **firstPZChampion ×3.32 — the draw, which the design does not set.** **sparksAfterPZ ×1.99 — the part the designer controls.** sparks ×2.01 — the two together. **Steer on `sparksAfterPZ` and retire `sparks` as a target.** The three `[draw]` conditions are printed, reported and not counted as failures. |
| Live suites | **36 suites, 1,955 `check()` call sites, 2,004 assertions, all passing** under `node tools/run-suites.mjs --selftest`. `tests/test-v65.mjs` is new, **48 assertions**. **Six superseded call sites re-pointed — build report §7**; four of them referenced the deleted `DISCOVERY_KNOWLEDGE_SET` and share a shim that derives it from `UPGRADES`. |
| **PARITY LEDGER — FINISHED** | **229 rows — PARITY 88, EASIER 118, HARDER 23, UNVERIFIED 0.** The Longhouse moved **PARITY → HARDER** on the restored provisions component; four new Discoveries added (huntersDraw PARITY, latchAndLever PARITY, arclightLance EASIER, leylineLensing EASIER). **The RR-ORIGINAL+UNVERIFIED guard ABORTED the build** until all four carried argued verdicts. |
| **A SOURCE CORRECTION — `railgun` is science 150,000, not "12,000-class"** | The v0.65 spec called it *"the source's science 12,000-class successor"*. Read from the pinned clone it is **science 150,000 on a 185,000 rung** (`particlePhysics`). RR's ladder tops at 135,000, so **no rung match exists** and the third weapon rung ships on `atlasGauntlets` (90,000) as a **ROLE match**, ledgered EASIER. |
| **A DECOMPOSITION INFERRED FROM A TOTAL IS NOT A DECOMPOSITION** | The spec's §2.3 gave four contributors to vigor's Σ. **The sum was exactly right at 5.4432 and every component was wrong** — Training Ground **73.5%** not 93.0%, cloud drake **0.948** not 0.231, champion passives **0.375** not 0.150, and `policyBoost("vigor")` **0.12** where the spec asserted the branch does not exist. `knee._sources` caught all four **on its first run, before a constant moved.** |
| **MANA IS COMPENSATED, NOT SOLVED** | v0.64's deficits are gone on the shipped build (**+26.43/s at hexcore, +24.81/s at final**), which discharges v0.64's condition 17. **But E1 — which contains the new mana Discovery — reads −4.10/s at hexcore, DEEPER than v0.64's −0.40/s**, because Part 2 cut the gross that the +25% multiplies. **Any round that restores vigor income must re-measure mana.** |
| **ZERO FAMILIES PAST THE KNEE, second round running** | Rails unchanged — vigor 8.0, devotion 5.0, provisions 3.0, mana 2.0. **vigor now has the most headroom of any family in the game: Σ 2.3363 against a knee of 6.00.** That is the arithmetic case for the next vigor move being an increase. |
| **`shimmer` AND `voidessence` STILL HAVE NO CONSUMER** | shimmer P/C **47.68**, 100% held; voidessence at ceiling with knowledge, culture and renown. **True since v0.57. Not a cap question (§24)** — it needs a sink or it needs deleting. |
| **OPEN FOR JERRY** | **(a)** Is vigor supposed to be the settlement's material income? **(b)** The **2,500-year horizon is an RR harness convention, not a source figure**, and it is now deciding the Icathia guard — seed 5 fails by not finishing inside a window nobody derived from Kittens. **(c)** The Longhouse provisions sink has a structural ceiling of **≈93,500** over a run; a bigger sink cannot ride on it. **(d)** §31 remains an open question with its premise retracted. |
| Cap families | **TWO.** `capFamilyOf()` returns `exempt` / `masonry` / null. |
| §30 | Reserved ids: `runestone`, `hunterLodge`, `lumberCamp`, `petricite`, `tavern`, `bloomery`, `refinedMetallurgy`, `kindling`, `championsRegimen`, `deepCartography`, `petriciteResonators`. |
| **Kittens retrieval** | **CLONE THE SOURCE.** `github.com/nuclear-unicorn/kittensgame` @ **`c52985b`**. Do not use grep.app. |
| Machine note | **`nproc` = 2.** v0.65's **5-seed** 2,500-year ensemble took **62 minutes** with the box to itself; v0.64's 3-seed run took 87 while sharing it with a slice chain. Budget 90–120 and run nothing else. Launch with `setsid nohup … & disown`. **And never point a long run at a file you still intend to edit** — kill by PID from `ps -eo pid,args`; §9 forbids `pkill -f`. |

...
| | |
|---|---|
| Last shipped build | **v0.61**, tagged `v0.61` — the converter stack decomposed, the ledger's RR-ORIGINAL backlog discharged, and a trade loop the source's own shape opened |
...
| **A GREP IS NOT A TEST** | v0.59's festival-banner assertion grepped the source for the string it had just written, passed for two rounds, and **the feature never fired once** — the chip read a wall-clock field v0.58 had set to zero forever. **Jerry reported the same note twice.** Any assertion satisfiable by the presence of text is not testing behaviour. |
| Cap families | **TWO.** `capFamilyOf()` returns `exempt` / `masonry` / null. Do not re-add a third. |
| Era 3 | **Reported, not scored** (v0.59 Part 3, Jerry's "907 is okay"). v0.60 measured **1,172.5, spread ×1.92**. v0.61's own ensemble figures are in BUILD REPORT §9. |
| **§31 — OPEN QUESTION, ruling requested** | *A category is a kind of effect, not an individual effect, and RR targets four.* **Nothing collapsed this round.** A converter output passes through ~11 multiplicative factors; grouping into four source-shaped categories cuts the product ~41%, collapsing to one cuts it ~80%. **It is a late-game change only and it wants its own round with the ensemble to itself.** **Until it is ruled, no round may add a new multiplicative category.** |
| **The converter stack — RE-MEASURED, and v0.60's finding REVERSES** | v0.60 reported ×19.77 "converter-side" against Kittens' ×3.70 and concluded RR runs ×5.3 the source. **That figure was `convMult × (1 + boosts.crystals)` — TWO categories multiplied — against ONE Kittens category.** `convMult` alone caps at ×5.3728 (×4.9335 after Part 1's additive collapse). Line to line: **RR's conversion upgrade line is 45% of the source's.** The excess is entirely the infernal drake (×1.495) and the overseer affinity (×2.000), both bounded, both EASIER. |
| **Trade yield — the source's shape does not transplant** | `js/diplomacy.js:744-747` sums `tradeRatio` additively and **uncapped**. Shipped uncapped, **RR grows an INFINITE-TIMBER LOOP at 133 Trade Docks**: RR has a Demacia → Piltover → transmute cycle Kittens has no analogue for, and the yield multiplier enters it twice. **A magnitude fix does not close it at any positive per-copy rate.** Shipped as ONE additive category with ONE ceiling (was four categories with two). **The real fix belongs to the transmute leg, not to trade.** |
| **`BOOST_LIMIT`'s 0.75·L knee bit for the first time** | `limitedDR` is linear only below 0.75·L. The mana line summed to **EXACTLY 0.75** with three members — the top of the linear region — so every member before v0.61 was delivered in full. **The fourth is the first that is not: Σ1.00 delivers 0.875.** **Check every other family for the same knee before adding a member.** |
| §30 | Reserved ids: `runestone`, `hunterLodge`, `lumberCamp`, `petricite`, `tavern`, `bloomery`, `refinedMetallurgy`, `kindling`, **`championsRegimen`, `deepCartography`**. |
| Tech ladder | **35 techs** (dev note 4 merged two bridge techs into The Vanguard Doctrine at 45,000). **RR's whole ladder is 1,442,630 knowledge against Kittens' 42,226,630 science — 3.4% — and tops out at 135,000 against 25,000,000.** Every branch tech is argued EASIER from that one measurement. |
| **Kittens retrieval** | **CLONE THE SOURCE.** `github.com/nuclear-unicorn/kittensgame` @ **`c52985b`**. grep.app silently drops its repo filter; raw fetches get summarised away. Pin the revision in every citation. |
| Machine note | **`nproc` = 2.** A three-seed 2,500-year ensemble needs the box to itself. **Do not run the suite runner and the ensemble concurrently** — they roughly double each other's wall time. Launch long runs with `setsid nohup … & disown`. |

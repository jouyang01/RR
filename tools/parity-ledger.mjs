...
const V = {
  ...
  automatedWorkshop:["js/workshop.js `factoryAutomation` (Workshop Automation)", "UNVERIFIED", "**REBUILT v0.59.1 NOTE 7.2 (Jerry): 'Automated Workshop discovery should work just like the Kitten's Workshop Automation upgrade.'** v0.58.1 shipped a YEARLY FLAT GRANT — one beam, slab, gear and plating per Manufactory, paid out of NOTHING — which is a faucet with no input and was rated EASIER on exactly that basis. Kittens' upgrade is a different KIND of effect: it watches the raw stockpiles and, when one is nearly full, converts the OVERFLOW into the crafted tier at the ordinary recipe price. **It is a spill-guard, not a faucet — it can only ever act on units that were about to be thrown away at the ceiling.** The SHAPE is now the source's and the 95% trigger is the documented threshold; the SHARE (5% of the ceiling per Manufactory per game-year) could not be retrieved from source this round and is an **RR-ORIGINAL magnitude, which is why this row is UNVERIFIED rather than PARITY.** Retrieving `factoryAutomation`'s conversion fraction is a cheap win for the next analyzer pass. Price 200 -> 900 crystals (note 7)"],

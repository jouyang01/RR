...
export async function runSim(page, years, seed = 1) {
  return await page.evaluate(({ years, seed }) => {
    ...
    function manageChampions() {
      // v0.60 PART 4 SLICE C/D — THE RENOWN-VARIANCE CHANNEL IS REMOVED.
      // Champion passives are production multipliers gated on a threshold crossing of a resource
      // v0.59 made deed-paced and therefore bursty. This forces the identical roster at the
      // identical year on every seed, so that channel contributes ZERO variance and whatever
      // spread survives is not it. Recruitment is otherwise disabled so the roster cannot drift.
      if (yearNow() >= 400) {
        for (const d of CHAMPS) if (!(S.champs[d.id] && S.champs[d.id].r)) {
          S.champs[d.id] = { r: 1, lvl: 0, xp: 0 };
        }
        if (!S.leader) makeLeader(CHAMPS[0].id);
        mark("firstChampion");
        return;
      }
      return;   // no recruitment at all before the fixed year — the channel is closed, not damped
